...
from ttkbootstrap import Button, Entry, Frame, Label, Separator
...
class JanelaRegistroDeVencedor(JanelaPadrao):
    def __init__(self, master: Frame, configuracoes, tabelas, **kwargs):
        ...
        Separator(master, orient=VERTICAL).place(
            relx=0.49, rely=0, relwidth=0.02, relheight=1
        )
    ...
    def _configura_formulario_de_vencedor(self, master: Frame):
        Label(
            master,
            text='Busca de participante',
            anchor=CENTER,
            **self.configuracoes.label_titulos,
        ).place(relx=0, rely=0.01, relwidth=1)

        conf_entrys = self.configuracoes.entry_parametros.copy()
        conf_entrys['state'] = READONLY

        Label(master, text='CPF:', **self.configuracoes.label_parametros).place(
        ...
        Label(master, text='Nome:', **self.configuracoes.label_parametros).place(
            relx=0.01, rely=0.305
        )
        Entry(master, name='entry_nome', **conf_entrys).place(
            relx=0.2, rely=0.3, relwidth=0.75
        )

        Label(master, text='CEP:', **self.configuracoes.label_parametros).place(
            relx=0.01, rely=0.405
        )
        Entry(master, name='entry_cep', **conf_entrys).place(
            relx=0.2, rely=0.4, relwidth=0.75
        )

        Label(master, text='Endereço:', **self.configuracoes.label_parametros).place(
            relx=0.01, rely=0.505
        )
        Entry(master, name='entry_endereco', **conf_entrys).place(
            relx=0.2, rely=0.5, relwidth=0.75
        )

        Label(master, text='Email:', **self.configuracoes.label_parametros).place(
            relx=0.01, rely=0.605
        )
        Entry(master, name='entry_email', **conf_entrys).place(
            relx=0.2, rely=0.6, relwidth=0.75
        )

        Label(master, text='Telefone:', **self.configuracoes.label_parametros).place(
            relx=0.01, rely=0.705
        )
        Entry(master, name='entry_telefone', **conf_entrys).place(
            relx=0.2, rely=0.7, relwidth=0.75
        )
        ...
        Button(
            ...
            command=lambda s=self.lista_de_sorteios: self.tabelas.exportar_relatorio_vencedores(
                s
            ),
    ...
    def _atualiza_sorteios(self, _):
        ...
        for sorteio in self.lista_de_sorteios:
            ...
            titulo = (
                nome_sorteio
                if not sorteio.nome_vencedor
                else f'{nome_sorteio} - {sorteio.nome_vencedor}'
            )
            Label(
                subframe,
                text=titulo,
                name=f'label_{nome_sorteio.lower()}',
                **self.configuracoes.label_titulos,
            ...
            Button(
                subframe,
                text=f'Abrir {nome_sorteio}',
                command=lambda s=nome_sorteio: self._selecionar_sorteado(s),
            ).grid(row=bt_row, column=1, padx=(0, 20))

            Separator(self.form_sorteios, orient=HORIZONTAL).pack(
                fill=X, expand=True, ipady=3, anchor=CENTER
            )
    ...
    def _selecionar_sorteado(self, nome_sorteio):
        ...
        sorteio_widgets = self.form_sorteios.children.get(
            f'frame_{nome_sorteio.lower()}'
        )
        if sorteio_widgets:
        ...
        self.btn_salvar.configure(
            command=lambda n=nome_sorteio: self._registra_vencedor(n)
        )
    ...
    def _registra_vencedor(self, nome_sorteio: str):
        ...
        frame_sorteio.children.get(f'label_{nome_sorteio.lower()}').configure(
            text=f'{nome_sorteio} - {nome_ganhador}'
        )
        for sorteio in self.lista_de_sorteios:
