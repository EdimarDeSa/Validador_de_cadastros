...
class JanelaImpressao(JanelaPadrao):
    ...
    def escreve_resultado_de_verificacao(self):
        ...
        self.master.after(500,
                          lambda: self.master.children.get('bt_inicia_impressao').place(relx=0.26, rely=self.linha_6))
    ...
    def inicia_thread_impressao(self):
        if not self.impressao.get_lista_de_impresoras_em_uso():
            Messagebox.show_warning(title="Impressora", message="Selecione uma impressora.")
            return
    ...
    def inicia_impressao(self):
        log_panel = PainelDeLogs(self.impressao, self.tabelas, **self.configuracoes.label_parametros)
