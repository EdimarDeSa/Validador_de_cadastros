...
from ttkbootstrap import Button, DateEntry, Entry, Frame, Label, StringVar, Toplevel
...
class JanelaSorteios(JanelaPadrao):
    def __init__(self, master: Frame, configuracoes, tabelas, **kwargs):
        ...
        self.frame_de_sorteios.place(
            relx=0.005, rely=0.51, relheight=0.49, relwidth=0.99
        )
    ...
    def inicia_ui_produtos(self, master: Frame):
        ...
        Button(
            ...
            command=self.abre_formulario_de_produto,
            style=DEFAULT,
    ...
    def inicia_ui_sorteios(self, master):
        self.tela_retratil_sorteios = CollapsingFrame(
            master, autohide=True, bootstyle=ROUND
        )
        self.tela_retratil_sorteios.place(relx=0, rely=0, relwidth=0.79, relheight=1)

        self.var_sorteio_atual = Sorteio(
            self.tela_retratil_sorteios, self.checa_nome_do_sorteio
        )

        Button(master, text='Importar sorteios', state=DISABLED, style=PRIMARY).place(
            relx=0.8, rely=0.3, relwidth=0.19
        )

        Button(master, text='Exportar sorteios', state=DISABLED, style=INFO).place(
            relx=0.8, rely=0.5, relwidth=0.19
        )
    ...
    def abre_formulario_de_produto(
        ...
        quantidade='1',
        centro_de_custos='',
        data_de_fechamento_cc='',
        responsavel_pelo_cc='Marta Rosa Nunes',
        iid=None,
    ):
        ...
        ).place(x=10, y=40, width=148)
        Button(self.top, image=self.imagens.img_lupa, command=self.busca_produto).place(
            x=162, y=37, width=48
        )
        ...
        Entry(
            self.top,
            name='campo_nome_do_produto',
            **self.configuracoes.entry_parametros,
        ).place(x=220, y=40, width=410)
        ...
        Entry(
            self.top,
            name='campo_centro_de_custos',
            validatecommand=(self.reg_verifica_digito_numerico, '%P'),
            validate='key',
            **self.configuracoes.entry_parametros,
        ).place(x=10, y=140, width=200)
        self.top.children.get('campo_centro_de_custos').insert(0, centro_de_custos)
        self.top.children.get('campo_centro_de_custos').bind(
            '<Key>', self.verifica_info
        )

        data_inicial = (
            datetime.strptime(data_de_fechamento_cc, '%d/%m/%Y')
            if data_de_fechamento_cc
            else None
        )
        Label(
            self.top,
        ...
        DateEntry(
            self.top, name='campo_data_de_fechamento_cc', startdate=data_inicial
        ).place(x=220, y=140)
        self.top.children.get('campo_data_de_fechamento_cc').bind(
            '<Key>', self.verifica_info
        )
        ...
        Entry(
            self.top,
            name='campo_responsavel_pelo_cc',
            **self.configuracoes.entry_parametros,
        ).place(x=440, y=140, width=350)
        self.top.children.get('campo_responsavel_pelo_cc').insert(
            0, responsavel_pelo_cc
        )
        self.top.children.get('campo_responsavel_pelo_cc').bind(
            '<Key>', self.verifica_info
        )
    ...
    def limpa_formulario_de_produto(self):
        campos = [
            ...
            'campo_quantidade',
            'campo_centro_de_custos',
            'campo_responsavel_pelo_cc',
        ]
        for campo in campos:
    ...
    def salva_produto(self, iid=None, event: Event = None):
        ...
        if not nome_do_produto:
            self.informa_falta_de_dado(
                'Nome do produto', self.top.children.get('campo_nome_do_produto')
            )
        ...
        if not quantidade:
            self.informa_falta_de_dado(
                'Quantidade', self.top.children.get('campo_quantidade')
            )
        ...
        if not cc:
            self.informa_falta_de_dado(
                'Centro de Custo', self.top.children.get('campo_centro_de_custos')
            )
            self.top.children.get('campo_centro_de_custos').focus_set()
            return

        data_fechamento_cc = self.top.children.get(
            'campo_data_de_fechamento_cc'
        ).entry.get()
        responsavel_cc = self.top.children.get('campo_responsavel_pelo_cc').get()
        if not responsavel_cc:
            self.informa_falta_de_dado(
                'Responsável pelo cc',
                self.top.children.get('campo_responsavel_pelo_cc'),
            )
            self.top.children.get('campo_responsavel_pelo_cc').focus_set()
            return

        continuar = Messagebox.yesnocancel(
            title='Continuar cadastros?',
            parent=self.master,
    ...
    def editar_produto(self):
        ...
        self.abre_formulario_de_produto(
            *self.tabela_de_produtos.get_row(**kwargs).values, **kwargs
        )
    ...
    def salva_sorteio(self):
        premios = [
            Produto(*linha.values) for linha in self.tabela_de_produtos.get_rows()
        ]
        ...
        if not self.var_sorteio_em_edicao:
            self.tela_retratil_sorteios.add(
                self.var_sorteio_atual,
                self.checa_nome_do_sorteio,
                edita_sorteio=self.edita_sorteio,
            )
    ...
    def busca_produto(self, event: Event = None):
        ...
        if produto is None:
            Messagebox.show_error(
                title='Planilha de produtos invalida',
                message='Planilha de produtos no formato inválido',
            )
            return

        self.top.children.get('campo_nome_do_produto').delete(0, END)
        self.top.children.get('campo_nome_do_produto').insert(
            0, '' if isinstance(produto, list) else produto.iloc[0]
        )
    ...
    def informa_falta_de_dado(campo: str, parent=None):
        Messagebox.show_warning(
            ...
            message=f'É necessário preencher o campo: {campo}',
            parent=parent,
        )
