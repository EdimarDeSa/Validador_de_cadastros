...
from .constants import CODECS, CPF, ENDERECO, EXTENSAO_DEFAULT, EXTENSOES, SEPARADORES
...
class Arquivos:
    ...
    def abre_documento(titulo='') -> Path:
        ...
        return (
            Path(diretorio_documento).resolve() if '.' in diretorio_documento else None
        )

    @staticmethod
    def salva_arquivo_filtrado(
        arquivo: pd.DataFrame, caminho: str = None, tipo: str = None
    ):
        if caminho is None:
        ...
        def try_open_csv(encoding, sep):
            try:
                ...
                if tipo_documento == '.csv':
                    df = pd.read_csv(
                        diretorio_documento, encoding=encoding, dtype='string', sep=sep
                    )
        ...
        for codec in CODECS:
            for sep in SEPARADORES:
                ...
                if df is not None:
                    return df
        showwarning(
            'Nenhum codec encontrado',
            f'Não foi possível achar o campo "{CPF}" com nenhum codec disponível',
        )
        return pd.DataFrame()
