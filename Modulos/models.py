...
class Tabelas:
    ...
    def inicia_tb_inscritos(self):
        caminho = self.arquivo.abre_documento()
        if not caminho:
            return
        df = self.arquivo.abre_dataframe(caminho)
        self.__tb_inscritos = df
        
        self.__tb_inscricoes_validas = pd.DataFrame(columns=df.columns)
    ...
    def __busca_enderecos(self):
        ...
        self.__tb_inscricoes_validas = self.__tb_inscricoes_validas.merge(informacoes_cep, right_index=True, left_index=True)

        self.busca = True
        
    @property
    def get_total_cadastros_repetidos(self):
        return self.__total_cadastros_repetidos
    
    @property
    def get_total_cpfs_invalidos(self):
        return self.__total_cpfs_invalidos
    
    @property
    def get_total_colaboradores_cadastrados(self):
        return self.__total_colaboradores_cadastrados

    @property
    def get_total_inscricoes_validas(self):
        return self.__tb_inscricoes_validas.shape[0]
    
    @property
    def get_estado_dos_inscritos(self):
